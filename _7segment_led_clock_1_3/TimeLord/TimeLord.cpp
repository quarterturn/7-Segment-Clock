...
void TimeLord::Adjust(uint8_t * when, long offset){
	offset += when[tl_minute];
	when[tl_minute] = Absolute(offset) % 60;
	offset /= 60;
	
	offset += when[tl_hour];
	when[tl_hour] = Absolute(offset) % 24;
	offset /= 24;
	...
	if(when[tl_day]<1){
		...
		when[tl_day] = LengthOfMonth(when);
		return;
	}
	
	if(when[tl_day] <= LengthOfMonth(when) ) return;
